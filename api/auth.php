...
switch ($action) {
    ...
    case 'profile':
        ...
        $stmt = $db->prepare("SELECT id, name, phone, telegram, role, COALESCE(email,'') email, created_at, COALESCE(company_name,'') company_name, COALESCE(inn,'') inn, COALESCE(kpp,'') kpp, COALESCE(legal_address,'') legal_address FROM users WHERE id = ?");
        $stmt->execute([$uid]);
        $user = $stmt->fetch();
